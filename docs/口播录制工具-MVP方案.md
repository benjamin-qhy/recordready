...
首批按已确认范围验证 Apple Silicon，Intel 延后；当前实测环境为 M4 Pro / macOS 26.4，macOS 13 的实验编译目标不构成最低系统兼容承诺。Windows 11 x64 目标保留，实机验证暂缓。最终最低系统版本由选用接口和实际验收共同确定。版本固定于锁文件。正式分发分别完成 macOS 签名公证和 Windows 安装包签名。
