...
final class Chart: NSView {
    var timer: Timer?
    let sound = NSSound(contentsOfFile: "/System/Library/Sounds/Ping.aiff", byReference: false)
    var lastPulse = -1
    var flashUntil = 0.0
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        // Short pulse every ten host-clock seconds. This tests end-to-end paths,
        // including speaker, microphone and rendering latency; not a calibrated source.
        let pulse = Int(t / 10)
        if pulse != lastPulse {
            lastPulse = pulse
            flashUntil = t + 0.25
        ...
        (t < flashUntil ? NSColor.white : NSColor(calibratedRed: 0.05, green: 0.08, blue: 0.16, alpha: 1)).setFill(); bounds.fill()
